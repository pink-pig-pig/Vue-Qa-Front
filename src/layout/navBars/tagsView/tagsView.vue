...
const isActive = (v: RouteItem) => {
	if (getThemeConfig.value.isShareTagsView) {
		...
		if ((v.query && Object.keys(v.query).length) || (v.params && Object.keys(v.params).length)) {
			...
			// https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I51RS9
			return v.path === state.routePath;
		}
	}
};
...
const addTagsView = (path: string, to?: RouteToFrom) => {
	// 防止拿取不到路由信息
	nextTick(async () => {
		// 修复：https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I3YX6G
		let item: RouteItem;
...
// https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I61VS9
...
// https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I5K3YO
// https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I61VS9
const transUrlParams = (v: RouteItem) => {
	...
	if (v.meta?.isDynamic) {
		 ...
		 * 2、通过事件调用时，路由的 path 不是原始定义的路由格式，如：/params/dynamic/details/Vue-Qa-Front/111/我是动态路由测试tagsViewName(非国际化)
		 *
...
// 拖动问题，https://github.com/pink-pig-pig/Vue-Qa-Front/issues/I3ZRRI
